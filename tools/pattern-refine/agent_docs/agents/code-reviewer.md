...
在 PatternRefine 项目中，你不是普通代码风格审查员。你还必须审查：

- 实现是否仍围绕当前项目正确主线：reference-guided production reconstruction。
- 实现是否错误地继续强化 outline refinement、scan-only centerline、`cleaned.svg` 或 `refined.pdf` 作为主交付逻辑。
- 结果是否服务于最终 SVG 和逐裁片 `0.2mm max deviation` 验收。
- 任何 `debug-pass`、结构接近、object count 接近被包装成“通过”的表述都应视为严重问题。
